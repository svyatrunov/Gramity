...
function planToStrategy(plan: Plan) {
  return {
    ...
    status: plan.active ? ("active" as const) : ("paused" as const),
...
export async function buildMiraPortfolio(telegramId: number) {
  ...
  let portfolioData = {
    totalUsd: 0,
    breakdown: { ton: 0, tston: 0, usdt: 0, lp: 0 },
  };
  try {
  ...
  const usdtBalance = depositAddress
    ? await getUsdtBalance(depositAddress).catch(() => 0)
    : 0;
  const tonBalanceUsd =
    portfolioData.breakdown.ton + portfolioData.breakdown.tston;
  ...
  return {
    ...
    withdrawal_address: withdrawalAddress
      ? maskAddress(withdrawalAddress)
      : null,
    withdrawal_address_masked: withdrawalAddress
      ? maskAddress(withdrawalAddress)
      : null,
